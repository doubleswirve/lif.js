...
stateful(
  ...
  dispatch => (_, {err, items, pending}) => html`
    ...
    ${pending ? html`<p>loading&hellip;</p>` : ''}
    ${
      err
        ? html`<p>${err}</p>`
        : ''
    ...
    ${
      !err && items
              ...
              <li>
                ...
                ${
                  by.length < 1
                    ...
                    : by.map((author, index) => {
                      const pre = index === 0 ? '' : ', '
                      return html`${pre}<span>${author}</span>`;
                    })
                }
                <span>${year}</span>
              </li>
        ...
        `
        : html`<p>no items; click to load data</p>`
    }
  `,
...
render(
  html`<simple-fetch></simple-fetch>`,
